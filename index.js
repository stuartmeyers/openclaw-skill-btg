...
function resolveLocalIdentityConfig(pluginConfig, agentId) {
    ...
    const displayName = typeof identity.displayName === "string" && identity.displayName.trim()
        ...
        : undefined;
    const stateDir = typeof identity.stateDir === "string" && identity.stateDir.trim()
...
async function runBtgCommand(rawCommand, localIdentity) {
    ...
    const normalized = commandText.toLowerCase().startsWith("btg ") || commandText.toLowerCase() === "btg"
        ? commandText
        : `btg ${commandText}`;
    ...
    register(api) {
        api.registerCommand({
            ...
            handler: async (ctx) => {
                ...
                const localIdentity = resolveLocalIdentityConfig(api.pluginConfig, agentId);
        ...
        api.registerTool((ctx) => {
            const localIdentity = resolveLocalIdentityConfig(api.pluginConfig, ctx.agentId);
            return {
                name: "btg_runner",
                description: "Run a raw BTG command through bash run_btg.sh and return stdout/stderr.",
                parameters: {
                    type: "object",
